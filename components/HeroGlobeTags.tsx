...
// Parallax strength — matches Vanta's camera sensitivity
...
export default function HeroGlobeTags() {
  ...
  const handleMouse = useCallback((e: MouseEvent) => {
    // Normalize to -1..1 centered on viewport — same coordinate space Vanta uses
    mouseRef.current = {
  ...
  useEffect(() => {
    const animate = (time: number) => {
      ...
      // Smooth mouse — same lerp approach Vanta uses (mouseEaseX/Y with 0.05 factor)
      const sm = smoothMouseRef.current;
      const m = mouseRef.current;
      smoothMouseRef.current = {
        x: sm.x + (m.x - sm.x) * SMOOTHING,
        y: sm.y + (m.y - sm.y) * SMOOTHING,
      };

      const vw = window.innerWidth;
      const vh = window.innerHeight;

      // Globe visual center = viewport center (Vanta canvas is fixed inset-0)
      // Add parallax offset that matches Vanta's camera movement direction
      const parallaxX = smoothMouseRef.current.x * PARALLAX_MAX;
      const parallaxY = smoothMouseRef.current.y * PARALLAX_MAX;
      const centerX = vw / 2 + parallaxX;
      const centerY = vh / 2 + parallaxY;

      // Orbit radius — scale with viewport
      ...
      for (let i = 0; i < TAGS.length; i++) {
        ...
        // Depth — sin(angle): positive = front, negative = back
